...
experiment_name = 'annealing_1'
load_file_dir = "/global/home/users/yifengh3/VAE/B_results/method2_beta2/end_beta_checkpoint"
load_file_name = "model_weights_end_5810_1.8e-01.hdf5"

beta_set = np.logspace(-5,1,25)[:-5]
betas = np.concatenate(
        (np.flip(beta_set[-9:-3]),
        beta_set[-8:-2],
        np.flip(beta_set[-9:-3]))
        )
...
plt.semilogy()
plt.savefig(osp.join(train_output_dir,"beta_schedule.png"))
...
model_file = osp.join(load_file_dir,load_file_name)
print("loading model at {}".format(model_file))
vae.load_weights(model_file)
init_epoch = get_epoch(load_file_name)
